function [Ks, t_k] = X2Ks_large_spl_final(X, k, blockSize)
%
%
[nSmp, nFea] = size(X);
...
K = bsxfun(@rdivide, K, sum(K, 1));
K = .5 * K + .5 * K';
% K = K'; % row-normalized
% K = bsxfun(@times, K, max(sum(K, 1), eps).^(-.5));
% K = .5 * K + .5 * K';
